...
export const ModalWindow: React.FC = () => {
   ...
   const actionModal = () => () => {
      if (!error && (dataModal.get && dataModal.get.input ? newName.get !== '' : true)) {
         closeModal()
         ...
         <section className='modal'>
            <div className='modal__content'>
               ...
               {dataModal.get.input &&
                  <input
                     required
                     onChange={({ target }) => { validString(target.value) }}
                     className={`modal__input ${error ? "modal__input--error" : ""}`}
                     type="text"
                     placeholder='New name'
                  />
               }
               {error && dataModal.get.input && <span>Поле не может быть пустым</span>}
               <div className='modal__actions'>
                  <button onClick={actionModal()} type='button'>True</button>
                  <button onClick={() => closeModal()} type='button'>False</button>
               </div>
            </div>
         </section>,
         portalRoot
